...
struct WindowCloseInterceptor: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSView {
        ...
        view.onAttach = { window in
            ...
            guard window.delegate !== context.coordinator else { return }
            context.coordinator.forwardee = window.delegate
...
final class CloseProxy: NSObject, NSWindowDelegate {
    ...
    weak var forwardee: NSWindowDelegate?

    func windowShouldClose(_ sender: NSWindow) -> Bool {
        // With more than one main window open, closing one should just close
        // it. Auxiliary windows carry different titles, so they don't count.
        let siblings = NSApplication.shared.windows.filter {
            $0 !== sender && $0.isVisible && $0.title == sender.title
        }
